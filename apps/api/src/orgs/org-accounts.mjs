 ...
 * @param {{ id: string, type: string, name: string, parent_id: string | null, structure?: string | null, status?: string, country?: string | null, legal_name?: string | null, created_at?: Date | string }} row
 */
export function toOrgAccount(row) {
  /** @type {{ id: string, type: string, name: string, parentId: string | null, status: string, structure?: string, country?: string, legalName?: string, createdAt?: string }} */
  const account = {
    id: row.id,
    type: row.type,
    name: row.name,
