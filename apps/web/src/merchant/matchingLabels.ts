...
export const MATCHING_MODE_CARDS = [
  ...
  },
  {
    ...
    blurb: "Main address unless same-amount conflict; then HD pool from public key — also fits concurrent same amounts.",
  },
] as const;

/** True when the mode cannot be chosen (Mode D is removed from Phase 1). */
...
export function matchingModeDisabledReason(mode: string): string | undefined {
  ...
  if (mode === MatchingMode.D || mode === "D") return MODE_D_PHASE1_UNAVAILABLE_REASON;
  return undefined;
}
