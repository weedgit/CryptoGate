...
**Against:** `doc/Watcher-Order-Status-Contract.md`, `@paymentgate/domain` `OrderStatus` / `PaymentOrderColumn` / `PaymentOrderAssignFields`  
**Date:** 2026-08-24
...
3. **Wire `assignOnCreate`** — replace `stubAssignOnCreate` (still mock address / always Mode B) with `@paymentgate/matching` once merchant settlement address is available (M2-12). Pass `listReservedPayableAmounts` for Mode C.
