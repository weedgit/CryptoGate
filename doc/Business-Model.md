# CryptoGate Business Model

Phase 1 is a merchant collection product. CryptoGate (the **platform**) sells software that creates payment orders and confirms on-chain receipts. It does not hold money, does not run an exchange, and does not convert crypto to fiat.
...
| --- | --- | --- |
| **Platform** | Operator / SaaS vendor (Stripe “platform”, CoinGate operator) | CryptoGate itself. Runs global policy, billing wallet, compliance. |
| **Agent account** | Channel partner / ISO-style reseller (common in payment processing) | Brings merchants onto CryptoGate. Sits under **Platform** or under another **agent account** (nested child shown as **Agent (sub) account** when parent context matters). Agent nesting depth is a **platform-wide setting** controlled by **Platform Owner** (default **2** in Phase 1: agent → agent (sub) → merchant). |
...
| --- | --- | --- |
| Company (for CryptoGate or customer) | **Platform** / **merchant account** | “Company” is ambiguous (vendor vs customer vs legal entity). |
...
The payer sends from their own wallet. Coins go straight to a wallet the **merchant account** controls. CryptoGate never sits in the middle of payer funds.
...
**Platform.** CryptoGate operator. Platform **Owner**, **Administrator**, and **Viewer** users manage global fee policy, agent onboarding, compliance override, billing wallet, and audit (see [Roles and permissions](#roles-and-permissions)).
...
2. The platform returns QR and payment link. Receive address is the merchant or site address (parent default or approved site override).
...
Payer ──on-chain──► Merchant wallet (100%)
         ▲
         └── CryptoGate watches only (no custody, no skim)
```
...
Network fees on payer payments are paid by the sender, not by CryptoGate.
