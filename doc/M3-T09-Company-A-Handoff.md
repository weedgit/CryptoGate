...
**After you reply**, we schedule a deploy window: migrate DB (through **052** on current `main`), start API + watcher, run smoke (health, sample order, webhook test). Staging Cashier APK install: [M5-08](M5-08-Cashier-Apk-Install.md).
...
| 3 | `pnpm --filter @paymentgate/api migrate` (through **052**) | no error |
