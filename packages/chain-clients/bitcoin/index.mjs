...
export async function healthCheck() {
  ...
  return {
    ok: true,
    network: "bitcoin",
    mode: cfg.configured ? "bitcoin-api" : "stub",
...
export async function listRecentTransfers(options = {}) {
  ...
  if (watched.length === 0) {
    return { transfers: [], mode: "bitcoin-api", watchedAddressCount: 0 };
  }

  try {
    const live = await fetchBitcoinTransfersForAddresses({
      watchedAddresses: watched,
      fetchImpl: options.fetch,
      runtimeConfig: cfg,
      pollMode: "bitcoin-api",
    });
    return {
