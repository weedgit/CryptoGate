/**
 * Arbitrum One chain client — ERC-20 via JSON-RPC when ARBITRUM_RPC_URL is set.
 * Supports enabled registry pairs on arbitrum_one (USDT, USDC).
...
/** @typedef {{ ok: boolean, network: string, mode: string, rpcConfigured: boolean, asset: string }} ArbitrumHealth */

/**
 * @returns {Promise<ArbitrumHealth>}
 */
export async function healthCheck() {
  const cfg = getArbitrumOneRuntimeConfig(AssetCode.USDT);
  return {
    ok: true,
    network: "arbitrum_one",
    mode: cfg.configured ? "arbitrum-rpc" : "stub",
...
export async function listRecentTransfers(options = {}) {
  ...
  const cfg = getArbitrumOneRuntimeConfig(asset);
...
export async function getTransactionConfirmationState(args) {
  ...
  const cfg = getArbitrumOneRuntimeConfig(args?.asset ?? AssetCode.USDT);
...
export function getArbitrumOneConfig(asset = AssetCode.USDT) {
  const cfg = getArbitrumOneRuntimeConfig(asset);
  return {
    network: cfg.network,
    asset: cfg.asset,
